...
#        if os.name == "nt":
#            conectado = self._conectar_tello_win()
    ...
    def current_image(self):
        if self.test_mode:
            ...
            dummy_image[height//3 : 2*height//3, width//3 : 2*width//3] = (0, 147, 255)
            return dummy_image
        else:
    ...
    def land(self):
        if self.test_mode:
            print("[DRONE TEST MODE] drone is landing now")
        else:
            return self.envia_comando("land")

    def takeoff(self):
        if self.test_mode:
            print("[DRONE TEST MODE] drone is flying now")
        else:
    ...
    def rc(self,x,y,z,yaw):
        if self.test_mode:
            print("[DRONE TEST MODE] drone is moving with speeds (%d, %d, %d, %d)" % (x, y, z, yaw))
        else:
            return self.envia_comando("rc %s %s %s %s"%(x,y,z,yaw))

    def goto(self,x,y,z,vel):
        if self.test_mode:
            print("[DRONE TEST MODE] drone is going to (%d, %d, %d) at speed %d" % (x, y, z, vel))
        else:
